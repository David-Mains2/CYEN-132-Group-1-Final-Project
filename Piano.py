	...
	while (True):
		...
		if (GPIO.input(play)):
                        ...
                        while(GPIO.input(play)):
                                sleep(0.01)
                        return "play"
                #finally chedk for the record button
...
def play_song():
        #each element in the song list is a list composed of
        #two parts: a note (or silence) and a duration
        for part in song:
                note, duration = part
                #if it's a silence, delay for its duration
                if (note == "SILENCE"):
                        sleep(duration)
                #otherwise, play the ntoe for its duration
                else:
                        notes[note].play(-1)
                        sleep(duration)
                        notes[note].stop()
...
#begin a non recording state and initialize the song
recording = False
song = []
...
try:
	while (True):
		#start a time
                start = time()
                #play a note when pressed .... until resleased
                ...
                #Otherwise a piano key was pressed
                        ...
                        start = time()
                        notes[key].play(-1)
                        print("{} is being played".format(key))
                        wait_for_note_stop(keys[key])
                        notes[key].stop()
                        #once the ntoe is released, stop the timer
                        duration = time() - start
